...
const LittleCardSelector = ({ isDisabled, originalValue, isSelector, options, label, inputName, register, watch, addClassName, type, isOnlyText, placeholder }) => {
    ...
    return (
        <div
            ...
            {isOnlyText ? <p className={inputClass}>{value}</p>
                : isSelector
                    ?
                    ...
                    :
                    <input value={value} autoComplete="off" disabled={isDisabled} type={type ?? "text"} style={{ border: "none", outline: "none" }} placeholder={placeholder} className={`${inputClass} ${isDisabled && "cursor-not-allowed"}`}
                        {...register(inputName)} />
            }
